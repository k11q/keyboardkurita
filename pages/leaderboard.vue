<template>
	...
	>
		<div class="w-full mb-10">
			<h2 class="font-mono text-4xl mb-6">Top Mode:Word</h2>
		...
		<div class="w-full">
			<h2 class="font-mono text-4xl mb-6">Top Mode:Time</h2>
			<template v-if="leaderboard && leaderboard.length">
				<table
					class="w-full font-mono text-neutral-200 flex flex-col gap-0.5 m-0"
				>
...
const leaderboardWord = computed(()=>{
	return leaderboard.value?.filter(i=>i.mode === 'word')
})

const leaderboardTime = computed(()=>{
	return leaderboard.value?.filter(i=>i.mode === 'time')
})
